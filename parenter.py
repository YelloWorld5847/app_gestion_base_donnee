...
colonnes_paranter = {"ID": 100, "Nom": 200, "Prénom": 150}

# Ajout du tableau à droite
tableau_parents = ttk.Treeview(tab3, columns=("ID", "Nom", "Prénom"), show='tree headings')

tableau_parents.tag_configure('evenrow', background='#330066')
tableau_parents.tag_configure('oddrow', background='#4d0099')

tableau_parents.heading("ID", text="ID")
tableau_parents.heading("Nom", text="Nom")
tableau_parents.heading("Prénom", text="Prénom")
tableau_parents.grid(row=0, column=4, rowspan=10, padx=10, pady=5, sticky="nsew")


# tableau enfant
tableau_enfants = ttk.Treeview(tab3, columns=("ID", "Nom", "Prénom"), show='headings')
tableau_enfants.heading("ID", text="ID")
tableau_enfants.heading("Nom", text="Nom")
tableau_enfants.heading("Prénom", text="Prénom")
tableau_enfants.grid(row=0, column=5, rowspan=10, padx=10, pady=5, sticky="nsew")

for tableau_paranter in [tableau_parents,tableau_enfants]:
    for value, width in colonnes_paranter.items():
        tableau_paranter.column(value, width=width, anchor=tk.CENTER)

# Configuration des colonnes pour redimensionnement
# tab3.grid_columnconfigure(4, weight=1)
# tab3.grid_rowconfigure(10, weight=1)

# Remplir le tableau avec des données d'exemple
for i in range(1, 21):
    true_or_no = random.choice((True, False))
    if i % 2 == 0:
        parent = tableau_parents.insert("", "end", values=(i, f"Nom {i}", f"Prénom {i}"), open=False, tags=('evenrow',))
    else:
        parent = tableau_parents.insert("", "end", values=(i, f"Nom {i}", f"Prénom {i}"), open=False, tags=('oddrow',))

    #parent = tableau_parents.insert("", "end", values=(i, f"Nom {i}", f"Prénom {i}"), open=true_or_no)
    for j in range(random.randint(0, 5)):
        tableau_parents.insert(parent,"end", text="Child 1", values=(j, f"Nom enfant {j}", f"Prénom enfant {j}"))


for i in range(1, 21):
    tableau_enfants.insert("", "end", values=(i, f"Nom {i}", f"Prénom {i}"))

def test(event):
    print("double clique tableau parent")
    print(tableau_parents.selection())
    print(tableau_parents.item(tableau_parents.selection()))

tableau_parents.bind("<Double-1>", test)
