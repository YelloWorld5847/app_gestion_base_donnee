...
def remplir_formulaire_entry(row):
    ...
    age_transform = str(int(annee_en_cours) - int(row[3]))
            ...
            for i in selected_item:
                ...
                if changes == 0:
                    ...
                    messagebox.showinfo("Confirmation",
                                        f"L'Utilisateur {item["values"][1]} {item["values"][2]} à bien été supprimé ")
    else:
        messagebox.showerror("Erreur", "Aucun identifiant sélectionnée")
...
def rechercher_adresse(combobox_resultats):
    global adresse
    adresse = combobox_resultats.get()

    if adresse:
        # URL de l'API d'adresse avec le paramètre "q" pour la recherche d'adresse
        url = f"https://api-adresse.data.gouv.fr/search/?q={adresse}&limit=5"
        ...
        if response.status_code == 200:
            data = response.json()
            ajouter_resultats(data, combobox_resultats)
        else:
            messagebox.showerror("Erreur", f"Erreur {response.status_code} lors de la requête à l'API.")
    else:
        messagebox.showwarning("Adresse manquante", "Veuillez entrer une adresse.")


# Fonction pour ajouter les résultats à la combobox
def ajouter_resultats(data, combobox_resultats):
    if data.get('features'):
        ...
        print(adresse3 + resultats)
        combobox_resultats['values'] = adresse3 + resultats
        combobox_resultats.current(0)
    else:
...
def open_new_window():
    ...
    child_windows[nomb_fenetre2] = {
        ...
        "type_combobox": ttk.Combobox(new_window,
                                      values=["Autiste", "Amis famille", "Prof", "Association", "Parent", "Bénévole",
                                              "Parent autiste", "Enfant"]),

        "activite_label": ttk.Label(new_window, text="Activité"),
        "activite_combobox": ttk.Combobox(new_window,
                                          values=["Music", "Café asperger ", "Théatre", "Chant", "GHS", "HS",
                                                  "Café asperger ado"]),
        ...
        "date_DateEntry": ttk.DateEntry(new_window, dateformat='%Y-%m-%d', firstweekday=2, startdate=dt,
                                        bootstyle=PRIMARY),
        ...
        "submit_button": ttk.Button(new_window, text="Valider",
                                    command=lambda: submit_form_enfant(nomb_fenetre2, id_parent)),
    ...
    widgets = [
        ...
        (child_windows[nomb_fenetre2]["code_postal_label"], 4, 0),
        (child_windows[nomb_fenetre2]["code_postal_entry"], 4, 1),
        (child_windows[nomb_fenetre2]["ville_label"], 5, 0), (child_windows[nomb_fenetre2]["ville_entry"], 5, 1),
        (child_windows[nomb_fenetre2]["telephone_label"], 6, 0),
        (child_windows[nomb_fenetre2]["telephone_entry"], 6, 1),
        ...
        (child_windows[nomb_fenetre2]["activite_label"], 1, 2),
        (child_windows[nomb_fenetre2]["activite_combobox"], 1, 3),
        (child_windows[nomb_fenetre2]["cotisation_label"], 2, 2),
        (child_windows[nomb_fenetre2]["cotisation_entry"], 2, 3),
        ...
        (child_windows[nomb_fenetre2]["mode_paiement_label"], 5, 2),
        (child_windows[nomb_fenetre2]["mode_paiement_combobox"], 5, 3),
        (child_windows[nomb_fenetre2]["date_label"], 6, 2), (child_windows[nomb_fenetre2]["date_DateEntry"], 6, 3),
        (child_windows[nomb_fenetre2]["commentaire_label"], 8, 0),
        (child_windows[nomb_fenetre2]["commentaire_text"], 8, 1, 1, 4),
        (child_windows[nomb_fenetre2]["membre_ca_check"], 7, 2),
        (child_windows[nomb_fenetre2]["entre_nous_check"], 7, 3),
...
# Menu déroulant pour saisir et afficher les résultats d'adresse
adresse_entry = ttk.Combobox(tab2, width=60)
adresse_entry.bind("<Return>", lambda event: rechercher_adresse(adresse_entry))  # Lier la touche "Entrée" à la fonction de recherche
...
type_combobox = ttk.Combobox(tab2, values=["Autiste", "Amis famille", "Prof", "Association", "Parent", "Bénévole",
                                           "Parent autiste", "Enfant"])

activite_label = ttk.Label(tab2, text="Activité")
activite_combobox = ttk.Combobox(tab2, values=["Music", "Café asperger ", "Théatre", "Chant", "GHS", "HS",
                                               "Café asperger ado"])
...
cotisation_entry.insert(tk.END, str(annee_en_cours))
...
btn_del = ttk.Button(tab4, text="Supprimer anciens membres", style="secondary.Toolbutton.TButton",
                     command=mise_a_jour_table)
